...
const MovieModal = () => {
  ...
  useEffect(() => {
    API.get(`/movie/${movieId}`).then((response) => {
      ...
      setGenres(
        response.data.genres.map((genre, i) => {
          return `${genre.name}${
            response.data.genres.length > ++i ? ", " : ""
          }`;
        })
      );
      setRuntime(
        response.data.runtime
          ? `${Math.floor(response.data.runtime / 60)}hr ${
    ...
    API.get(`/movie/${movieId}/credits`).then((response) => {
      setCast(
        response.data.cast.map((person, i) => {
          return <CastList key={i} person={person} />;
        })
      );
    });
  ...
  function getTrailer() {
    API.get(`/movie/${movieId}/videos`).then((response) => {
  ...
  return (
    <motion.div
      className="absolute bg-neutral-800 rounded-xl
       text-neutral-100 w-[700px] h-96 drop-shadow-2xl"
      animate={controls}
    >
      ...
      {!flipped ? (
        <div className="flex">
          <div
            className="flex-shrink-0 h-96 relative cursor-pointer"
            onClick={getTrailer}
          >
            <img
